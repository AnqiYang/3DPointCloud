...
flags.DEFINE_integer('epochs', 200, 'Number of epochs to train.')     #shz 200
# flags.DEFINE_integer('hidden1', 16, 'Number of units in hidden layer 1.')
# flags.DEFINE_float('dropout', 0, 'Dropout rate (1 - keep probability).')
# flags.DEFINE_float('order_num', 0, 'order number.')
flags.DEFINE_float('weight_decay', 5e-4, 'Weight for L2 loss on embedding matrix.')
flags.DEFINE_integer('early_stopping', 200, 'Tolerance for early stopping (# of epochs).')  #shz 200
...
def run(seed):
    ...
    swap_list = list()
    order_num = [2]   # revise by shz [1,2,3,4]
